(function () {
  ...
  // Definitive roster for The Dark Passage. Name highlighting and the
  // character picker recognise exactly these names (and only these).
  var ROSTER = [
    "space_fan", "CrystalWolf", "Schwalbe", "Duccarian", "Ashborn", "Shroud",
    "Rally Mally", "The Reddening Day", "Toad", "Retard 2.0", "CharliSpirit",
    "lumallie", "Galaxy", "Godly_Aura", "Polaris", "Revenant", "Elv", "Golden",
    "Loyal2Schwab", "Rabbit", "Drug", "janhk", "Kiwi", "Akarimmu", "JIKOSU",
    "Lyrix", "hollow Camillia", "NinjaFlout", "Mainthemainstar", "pinkfml",
    "Reaps", "Doomfox", "duck179", "HeyNoQuest", "Ink'dDiamondPaintingQueen",
    "Nightmare", "PopcornRya", "SomeDucks", "Acoustyx", "WavyDragons"
  ];
  ...
  function init() {
    ...
    discoverAvailableDays()
      .finally(function () {
        ...
        if (subtitle) {
          subtitle.textContent = "Descend through the days — switch day, reading mode, and theme from the controls.";
        }
      });
  }

  function bindEvents() {
    if (readerFactionSelect) readerFactionSelect.addEventListener("change", function () {
  ...
  function applyReaderFactionTheme(key) {
    // Factions removed: the palette is driven entirely by the stylesheet
    // (:root + [data-era] in passage.css), so we no longer override colours
    // per faction here. The data-era attribute (see updateReaderEraTheme)
    // selects the day3+ tone within the Ashfall theme.
    if (document.body) {
  ...
  function applyNameColorMode(mode) {
    // "prefix" (team prefix) was removed with factions; fall back to "color".
    var validMode = mode === "none" ? "none" : "color";
  ...
  function isReaderFactionHighlightMode() {
    // Factions removed: the faction-highlight focus mode no longer exists.
    return false;
  }
  ...
  function getCharacterPickerMembers() {
    // Factions removed: the character picker lists every known member.
    return getAllKnownMembers();
  }

  function getAllKnownMembers() {
    // The character picker lists exactly the fixed roster (deduped, A–Z).
    var seen = {};
    var members = [];
  ...
  function renderInfiniteReader() {
    ...
    readerTitle.textContent = "Infinite Reader";
    readerMeta.textContent = "All available days";
    updateReaderOptionalSettingsVisibility();
    updateReaderEraTheme();
  ...
  function renderStructuredDay(text, factionKey, dayNumber) {
    ...
    if (shouldUseTeamTags(dayNumber)) {
      // Factions removed: render every team's roster block as plain story
      // content (no faction selection/colouring), so early days that are
      // built entirely from team rosters still show in full.
      var teamOrder = ["astral-wardens", "obsidian-dominion", "velocity-syndicate"];
      var tagMap = {
        "astral-wardens": "TEAM_A_ASTRAL_WARDENS",
        "obsidian-dominion": "TEAM_B_FURIOUS_FLOOFS",
        "velocity-syndicate": "TEAM_C_VELOCITY_SYNDICATE"
      ...
      teamOrder.forEach(function (teamKey) {
        var teamTag = tagMap[teamKey];
        ...
        if (!leader) {
          leader = inferLeaderForFaction(teamKey, memberLines);
        }

        html += '<div class="reader-section reader-section--faction">';
        html += '<h4 class="reader-section-label">' + escapeHtml(FACTIONS[teamKey].name) + "</h4>";
  ...
  function rebuildMemberMatchersFromLoadedContent() {
    // The roster is fixed (see ROSTER): name highlighting recognises exactly
    // those names. The hunter day-colouring still works via HUNTER_NAMES_BY_DAY
    // in getFactionForNormalizedName.
    state.memberMatchers = buildMemberMatchers(ROSTER);
    state.memberFactionLookup = {};
  }
  ...
  function getMemberPrimaryColor(normalizedName, factionKey) {
    ...
    // Factions removed: give each name a stable, distinct colour drawn from a
    // warm ember band (amber -> orange -> rust) so highlighting stays on-theme
    // with The Dark Passage and never reintroduces the old cyan/blue.
    var hash = 0;
    for (var i = 0; i < normalizedName.length; i++) {
      hash = (hash * 31 + normalizedName.charCodeAt(i)) % 997;
    }

    var hue = 18 + (hash % 30); // 18deg (rust) .. 47deg (amber)
    var sat = 72 + (hash % 16); // 72% .. 87%
    var light = 56 + (hash % 10); // 56% .. 65%
